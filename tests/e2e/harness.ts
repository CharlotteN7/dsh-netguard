...
export interface AgentRunOptions {
  ...
  /**
   * Configuration for the `dsh-netguard` row, merged over the spool path this
   * harness owns. The row patch REPLACES the bundle layer's whole `config`, so
   * the spool path has to be restated here rather than left to `dshHomePath`.
   */
  readonly netguard?: Readonly<Record<string, unknown>>
  /**
...
export interface AgentRunResult {
  ...
  /** OCSF records the plugin spooled, in order. */
  readonly ocsfRecords: readonly OcsfLine[]
  /** Absolute path of the spool, for a test that reads it with the CLI. */
...
export async function runAgent(options: AgentRunOptions): Promise<AgentRunResult> {
  ...
  // isolates it; the row patch below restates it because a patch replaces a
  // row's whole config.
  const spoolPath = join(home, 'netguard', 'decisions.ocsf.jsonl')
  ...
  try {
    ...
    // YAML is a superset of JSON, so a flow mapping is the least error-prone
    // way to hand a nested config through the patch layer.
    const netguardConfig = JSON.stringify({ spoolPath, ...options.netguard })
